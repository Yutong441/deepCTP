...
ID <- closest_list2_to_list1

#---------- Additional processing not used by the authors---------- 
library (tidyverse)
keep_features <- c('list1_id', 'ylabel', 'M.F', 'i.Age', 'mmse', 'cdr',
                   'commun', 'homehobb', 'memory', 'judgment', 'orient',
                   'apoe', 'perscare', 'sumbox', 'height', 'weight')
ID %>% filter (!is.na (Label)) %>%
        filter (!is.na (dx1)) %>%
        mutate (dx1 = gsub ('No dementia', 'Cognitively normal', dx1)) %>%
        mutate (ylabel = ifelse (dx1 == 'Cognitively normal', 0, 1)) %>%
        select (all_of (keep_features)) %>%
        rename (experiment_id = list1_id) -> ID

ID %>% select (experiment_id) %>% 
        write.csv (file=gsub('all.csv', 'MR_ID.csv', output_filename), row.names=F, quote=F)
write.csv (ID, file=output_filename, row.names=F, quote=F)
